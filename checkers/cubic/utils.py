...
def check_encryption_correctnes(ct, pub_cube, priv_key, pt):
    ct_cubes = [tuple(map(Cube.from_str, x)) for x in loads(b64decode(ct))]
    cipher = Cipher(pub_cube, priv_key)
    res = cipher.decrypt(ct_cubes).decode()
    return res == pt
